...
CREATE INDEX IF NOT EXISTS idx_tiers_plan   ON plan_tiers (plan_id);
CREATE INDEX IF NOT EXISTS idx_tiers_active ON plan_tiers (is_active);
...
CREATE INDEX IF NOT EXISTS idx_sessions_alive     ON sessions (is_revoked, expires_at);

-- ── HWID history ──────────────────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS hwid_records (
    id         UUID              PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id    UUID              NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    hwid_old   TEXT,
...
CREATE INDEX IF NOT EXISTS idx_hwid_user ON hwid_records (user_id);
...
CREATE TABLE IF NOT EXISTS nonce_blacklist (
    nonce      TEXT        PRIMARY KEY,
    expires_at TIMESTAMPTZ NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_nonce_expires ON nonce_blacklist (expires_at);
...
CREATE TABLE IF NOT EXISTS plan_integrity (
    id            UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    plan_id       UUID        NOT NULL REFERENCES subscription_plans(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_integrity_plan ON plan_integrity (plan_id);
...
CREATE INDEX IF NOT EXISTS idx_re_session ON runtime_events (session_id);
CREATE INDEX IF NOT EXISTS idx_re_user    ON runtime_events (user_id);
CREATE INDEX IF NOT EXISTS idx_re_type    ON runtime_events (event_type);
CREATE INDEX IF NOT EXISTS idx_re_created ON runtime_events (created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_pe_partner ON partner_earnings (partner_id);
CREATE INDEX IF NOT EXISTS idx_pe_paid    ON partner_earnings (is_paid);

-- ── Loader keys ───────────────────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS loader_keys (
    id         UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_lk_plan  ON loader_keys (plan_id);
CREATE INDEX IF NOT EXISTS idx_lk_used  ON loader_keys (is_used);
CREATE INDEX IF NOT EXISTS idx_lk_value ON loader_keys (key_value);
